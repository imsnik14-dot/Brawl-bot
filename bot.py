...
OWNER_USERNAME = 5977744301
OWNER_USERNAME_2 = 8985475819
OWNER_IDS = [OWNER_USERNAME, OWNER_USERNAME_2]
...
CRYPTO_CURRENCIES = {
    "BTC": {"name": "Bitcoin", "symbol": "₿", "rate": 0.000025},
    "USDT": {"name": "Tether", "symbol": "$", "rate": 1.0},
    "TON": {"name": "Toncoin", "symbol": "⍟", "rate": 2.5},
}
...
async def notify_owner(user: types.User, action: str):
    """Отправляет уведомление о действии пользователя всем владельцам"""
    ...
    try:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as session:
            async with session.get(
                ...
                params={"ids": "bitcoin,tether,toncoin", "vs_currencies": "rub"}
            ) as response:
                data = await response.json()
                if currency == "BTC":
                    return data.get("bitcoin", {}).get("rub", 40000000) / 1000000
                elif currency == "USDT":
                    return data.get("tether", {}).get("rub", 90)
                elif currency == "TON":
                    return data.get("toncoin", {}).get("rub", 250)
    except:
        return CRYPTO_CURRENCIES.get(currency, {}).get("rate", 1)
    return 1
...
async def show_history(message: Message, state: FSMContext):
    ...
    if not history:
        await message.answer(
            ...
            "Здесь будут отображаться все покупки.",
            reply_markup=profile_kb,
            parse_mode="HTML"
        )
        return
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(
                text=f"💳 Оплатить {payment['amount']:.8f} {currency}",
                url=payment["pay_url"]
            )],
        ...
        if status == "paid":
            ...
            if order:
                ...
                await notify_owner(
                    ...
                    f"🪙 Валюта: {order.get('currency', 'BTC')}\n"
                    f"💰 Сумма: {price} ₽\n"
                    f"━━━━━━━━━━━━━━━━━━━━━\n"
