...
def log_action(user: types.User, action: str):
    ...
    if len(actions_log[user.id]) > 100:
        actions_log[user.id] = actions_log[user.id][-100:]
...
# ============================================
# ===== ФУНКЦИИ CRYPTOBOT (БЕЗ aiohttp) =====
# ============================================
def crypto_request(url, data):
    """Отправляет запрос к Cryptobot через urllib"""
    headers = {
...
def create_crypto_payment_sync(amount_rub: int, currency: str, description: str, user_id: int):
    """Создаёт платёж через Cryptobot (синхронная версия)"""
    # Фиксированные курсы для простоты
    ...
    result = crypto_request(f"{CRYPTOBOT_URL}/createInvoice", payload)
...
def check_crypto_payment_sync(invoice_id: int):
    """Проверяет статус платежа"""
    result = crypto_request(f"{CRYPTOBOT_URL}/getInvoices", {"invoice_ids": [invoice_id]})
...
async def create_crypto_payment(amount_rub: int, currency: str, description: str, user_id: int):
    return await asyncio.to_thread(create_crypto_payment_sync, amount_rub, currency, description, user_id)

async def check_crypto_payment(invoice_id: int):
    return await asyncio.to_thread(check_crypto_payment_sync, invoice_id)
...
async def show_history(message: Message, state: FSMContext):
    ...
    if not history:
        await message.answer("📜 <b>История операций пуста</b>\n\nЗдесь будут отображаться все покупки.", reply_markup=profile_kb, parse_mode="HTML")
        return
...
async def buy_account(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"💡 Курс будет рассчитан автоматически",
        reply_markup=keyboard,
        parse_mode="HTML"
    )
...
async def process_crypto_pay(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.edit_text(
            ...
            f"3️⃣ Нажмите «Проверить оплату» после перевода\n\n"
            f"⏰ <b>Срок оплаты:</b> 30 минут",
            reply_markup=keyboard,
            parse_mode="HTML"
        )
        await state.set_state(ShopStates.main_menu)
        ...
        await callback.message.answer(f"❌ <b>Ошибка создания платежа!</b>\n\nОшибка: {e}", reply_markup=main_menu_kb, parse_mode="HTML")
    ...
    try:
        ...
        if status == "paid":
            ...
            if order:
                ...
                await bot.send_message(user_id, f"✅ <b>Оплата прошла успешно!</b>\n\n🎯 <b>Товар:</b> {product_name}\n💰 <b>Сумма:</b> {price} ₽\n\nСкоро с вами свяжется оператор для передачи товара. 🎉", parse_mode="HTML")
                await notify_owner(callback.from_user, f"🪙 <b>НОВАЯ ОПЛАТА КРИПТОВАЛЮТОЙ</b>\n━━━━━━━━━━━━━━━━━━━━━\n👤 Пользователь: {callback.from_user.full_name} (@{callback.from_user.username})\n🆔 ID: {callback.from_user.id}\n🎯 Товар: {product_name}\n🪙 Валюта: {order.get('currency', 'BTC')}\n💰 Сумма: {price} ₽\n━━━━━━━━━━━━━━━━━━━━━\n📩 Свяжитесь с покупателем для передачи товара!")
                
                await callback.message.edit_text(f"✅ <b>Оплата подтверждена!</b>\n\n🎯 <b>Товар:</b> {product_name}\n💰 <b>Сумма:</b> {price} ₽\n\nСкоро с вами свяжется оператор.\nСпасибо за покупку! 🎉", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 В главное меню", callback_data="payment_cancel")]]), parse_mode="HTML")
            ...
            await callback.message.edit_text("❌ <b>Время оплаты истекло!</b>\n\nПопробуйте оформить заказ заново.", reply_markup=main_menu_kb, parse_mode="HTML")
...
async def process_support(message: Message, state: FSMContext):
    ...
    admin_text = f"📩 <b>Новое сообщение от пользователя</b>\n━━━━━━━━━━━━━━━━━━━\n👤 <b>Юзернейм:</b> @{user.username if user.username else 'без юзернейма'}\n📛 <b>Имя:</b> {user.full_name or 'без имени'}\n🆔 <b>ID:</b> <code>{user.id}</code>\n━━━━━━━━━━━━━━━━━━━\n\n<b>Текст сообщения:</b>\n{message.text}"
    ...
    try:
        ...
        await message.answer(f"❌ Ошибка при отправке: {e}\n\nПопробуйте позже", reply_markup=support_kb, parse_mode="HTML")
...
async def admin_password_check(message: Message, state: FSMContext):
    ...
    if message.text == ADMIN_PASSWORD:
        ...
        await message.answer("❌ <b>Неверный пароль!</b>\n\nПопробуйте ещё раз или нажмите 'Назад'.", reply_markup=back_kb, parse_mode="HTML")
...
async def admin_balance_amount_input(message: Message, state: FSMContext):
    ...
    try:
        await bot.send_message(target_user, f"💰 <b>Ваш баланс пополнен!</b>\n\nСумма: <b>+{amount} ₽</b>\n📊 Новый баланс: <b>{new_balance} ₽</b>\n\n👤 <b>Кто:</b> Администратор", parse_mode="HTML")
    except:
        pass
...
async def catch_all_messages(message: Message, state: FSMContext):
    ...
    if current_state == ShopStates.waiting_support:
        await message.answer("📩 Отправьте текстовое сообщение с вашим вопросом.\nИли нажмите 'Назад' для отмены.", reply_markup=support_kb)
    else:
        await message.answer("❓ Используйте кнопки меню.\nЕсли вы не видите кнопки, нажмите /start", reply_markup=main_menu_kb)
...
async def main():
    ...
    print("   - 👤 Профиль с балансом в ₽")
    print("   - 📦 Наличие товара с кнопками покупки")
    print("   - 🛒 Покупка аккаунта (899 ₽) через криптовалюту")
    print("   - 🛒 Покупка БП+ (199 ₽) через криптовалюту")
    print("   - ❓ Поддержка")
